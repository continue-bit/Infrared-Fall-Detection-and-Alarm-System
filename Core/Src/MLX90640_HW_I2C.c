...
int MLX90640_I2CRead(uint8_t slaveAddr, uint16_t startAddress, uint16_t nMemAddressRead, uint16_t *data)
{
    ...
    if(HAL_I2C_Mem_Read(active_hi2c, i2c_addr, startAddress, I2C_MEMADD_SIZE_16BIT, (uint8_t*)data, nMemAddressRead * 2, 500) == HAL_OK)
    {
...
int MLX90640_I2CWrite(uint8_t slaveAddr, uint16_t writeAddress, uint16_t data)
{
    ...
    if(HAL_I2C_Mem_Write(active_hi2c, i2c_addr, writeAddress, I2C_MEMADD_SIZE_16BIT, (uint8_t*)&data_swapped, 2, 500) == HAL_OK)
    {
        return 0; 
    }
    return -1; 
